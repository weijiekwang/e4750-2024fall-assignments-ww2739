...
if __name__ == "__main__":
    ...
    for current_operation in valid_operations:
        ...
        plt.plot(valid_vector_sizes, arr_avg_total_add_device_mem_gpu_time_incl, label='add_device_mem_gpu incl')
        plt.plot(valid_vector_sizes, arr_avg_total_add_host_mem_gpu_time, label='add_host_mem_gpu')
        plt.plot(valid_vector_sizes, arr_avg_total_add_gpuarray_kernel_time_incl, label='add_gpuarray_kernel incl')
        plt.plot(valid_vector_sizes, arr_avg_total_add_gpuarray_no_kernel_time_incl, label='add_gpuarray_no_kernel incl')
        plt.xscale('log')
        plt.yscale('log')
        plt.xlabel('Vector Size (Power of 10)')
        plt.ylabel('Average Execution Time (s)')
        plt.title('Average Execution Time Including Memory Transfer for ' + current_operation)
        plt.legend()
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(valid_vector_sizes, arr_avg_total_add_device_mem_gpu_time_excl, label='add_device_mem_gpu excl')
        ...
        plt.xlabel('Vector Size (Power of 10)')
        plt.ylabel('Average Execution Time (s)')
        plt.title('Average Execution Time Excluding Memory Transfer for ' + current_operation)
        plt.legend()
        plt.xticks(valid_vector_sizes, [str(int(np.log10(x))) for x in valid_vector_sizes])
        plt.grid(True)
    ...
    valid_vector_sizes = vector_sizes  # Now includes up to 10^8

    # Repeat the same analysis as above with updated valid_main_methods and valid_vector_sizes
    # Re-initialize arrays and perform the loops again (omitted here for brevity)
    # Remember to plot the results as per the requirements
